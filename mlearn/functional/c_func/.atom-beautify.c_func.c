...
// 全连接神经网络
double *matmulAdd(double *inputs, int inputs_row, int inputs_col, double *w, int w_row, int w_col, double *b, int b_row, int b_col)
{
  ...
  for (i = 0; i < inputs_row; i++)
  {
    for (k = 0; k < w_row; k++)
    {
      ...
      printf("%f  ", row_sum+b[k]);
      result[index++] = row_sum + b[k];
    }
  }

  index = 0;
  printf("\nResult\n");
  for(int i = 0; i < inputs_row; i++)
    for(int k = 0; k < w_row; k++)
      printf("%f ", result[i * w_row + k]);
    printf("\n");
...
void conv2d_test(int argc, char *argv[])
{
...
void dense_test()
{
  ...
  result = matmulAdd(inputs, 2, 3, w, 2, 3, b, 1, 3);
