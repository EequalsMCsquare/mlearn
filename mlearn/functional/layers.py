...
def dense(inputs: Tensor, weights: Tensor, bias: Tensor) -> Tensor:
    ...
    requires_grad = weights.requires_grad or bias.requires_grad

    depends_on = []
    if weights.requires_grad:
        def denseWeightBackward(grad):
            return inputs.data.T @ grad
        depends_on.append(Dependency(weights,denseWeightBackward))

    if bias.requires_grad:
        def denseBiasBackward(grad):
            ndims_added = grad.ndim - bias.data.ndim
            for _ in range(ndims_added):
                grad = grad.sum(axis=0)
            for i, dim in enumerate(bias.shape):
                if dim == 1:
                    grad = grad.sum(axis=i, keepdims=True)
            return grad
        depends_on.append(Dependency(bias, denseBiasBackward))
...
def conv_2d_experiment(inputs: Tensor, weights: Parameter,
    ...
    strided_batch = np.array([np.lib.stride_tricks.as_strided(x, view_shape, _strides) for x in _inputs.data])[:,0,0:out_x:stride[0],0:out_y:stride[1]]
    ...
    for arr in strided_batch:
        batch_result.append(
            c_func.sample_conv2d(arr, weights.data.reshape(1, *weights.shape), bias.data.reshape(-1, 1, 1, 1, 1))
        )
